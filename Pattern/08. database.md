...
- *I think you already sense this, but if you combine this `Database Layer` with [`Type Manipulation`](https://github.com/0x76agabond/chimera-architecture/blob/main/Pattern/05.%203-level-of-DI.md), [`Template Method`](https://github.com/0x76agabond/chimera-architecture/blob/main/Pattern/04.%20template.md), and the [`Kit Layer`](https://github.com/0x76agabond/chimera-architecture/blob/main/Pattern/02.%20use-cases.md), the result is a system where **changing table columns only requires updating the Kit and the Data Model** (likely 2 rows in my case).* 
...
*A. I'm a wandering reseacher*.

***Q. What does it ever mean?***  
*A. I go around researching things nobody asked for.*
...
***Q. I can still do SQL injection by putting malicious SQL inside a data-model attribute. What now?***  
*A. Every database provides a way to read its own metadata, such as information_schema in `PostgreSQL` or `SQL Server`. You can use this metadata to build a strict whitelist that rejects anything outside the allowed schema.*  
*But if a data model can smuggle raw SQL, the real issue is your code review process, not SQL injection.*.
