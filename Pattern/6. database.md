...
- `The Adapter` **may** use reflection and caching to handle table columns.

- `The Repository` enforce validation before pass to Database Adapter.

- `The Repository` provides table information (**it should know exactly which table is being use by current Use Cases**) and fills the condition block **using parameterized queries** only.

- If you're abusing `Dapper` like me, you probably love to abuse `Dapper.Contrib`
...
## **Note**
- **Yes this pattern seems like I try to handle Dynamic Query with duct-tape, but if your project is not too big and you work with customer who change their mind like weather, this is life saver pattern*
...
- **If you don’t use `Dapper`, you can still **get a similar effect with most base connectors (parameterization is a must) combined with** `Reflection`.*
...
*A. I go around and paint people's houses*.

***Q. Reflection add overhead!***  
*A. Only for the first guy who touches it (the poor CI runner)*.
*Write a warm-up script if it bothers you*.

***Q. Who is John?***   
*A. I don't know but sorry John*.