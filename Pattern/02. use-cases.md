...
### **2. `Flow`**
The orchestrator that coordinates multiple steps.  
It decides when, what, in what order, fallback, branching, and merging.  
Flow contains orchestration logic, but never business logic.
...
**4. `Validation` must not contain business logic.**  
*Validation is only allowed to check structure, format, and required fields*

**5. `Validation` is not counted as a `Service` call.**   
**You can stack as many validation steps as needed before the Service, but if you need 20 validations, your Service probably has bigger problems!*

**6. Don't wrap a `Repository` with another `Repository`, use a `Flow` instead.**  
...
## **Debates**
***Q. So you think Validation is not business?***  
*A. We should separate validation from business constraint checking.*  
*Validation makes sure the input is clean and usable.*  
*Business constraints belong to the domain rules, not the data gate.*  

***Q. So you think that CRY thing is clever? What if my Kit needs new properties later?***  
*A. Use a Data Object (struct/class) as the function parameter*.  
*This gives you one stable boundary where fields can evolve safely*.  
*I do this myself, Service file maps to one Data Object file, each function gets its own Data Object*.

***Q. How about the God Flow? What if I need to orchestrate 100 Services?***  
*A. Split the Flow into multiple smaller flows, each handling a meaningful segment*.  
*Make sure each step actually makes sense, or your teammates will end up writing an architecture doc about the Flow’s suffering*.  
*And if there is truly a reason you must orchestrate 100 Services, you probably need a God to do it for you*.
